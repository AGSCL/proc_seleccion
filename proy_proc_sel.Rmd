...
part_3%>%
  dplyr::slice(1:400) %>% 
  knitr::kable(format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
                caption = paste0("Tabla. Primeros 400 casos"),
                align =c('l',rep('c', 101)))%>%
     kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 12) %>%
     kableExtra::add_footnote(c("Nota. "),
                              notation = "none")%>%
...
fuzzyjoin::stringdist_join(data.frame(name=part_12$sel1), data.frame(name=part_12$sel1), 
  ...
  slice(1:20) %>% 
    knitr::kable(format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
               caption = paste0("Tabla. Comparación de nombres"),
               align =c('l',rep('c', 101)))%>%
    kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 12) %>%
  kableExtra::add_footnote(c("Nota. "),
                            notation = "none")%>%
...
```{r 18_desc1, echo=T, paged.print=TRUE,eval=T}
...
```{r 18_desc3, echo=T, paged.print=TRUE,eval=T}
part_17<-
part_16 %>% 
...
```{r 18_desc4, echo=T, paged.print=TRUE,eval=T}
...
```{r 18_desc5, echo=T, paged.print=TRUE,eval=T}
...
```{r 18_desc6, echo=T, paged.print=TRUE,eval=T}
...
```{r 20_survplot_sex_rec, dpi = 96, warning=F,message=F,fig.align='center', fig.cap="Figure 12. Cum. HRiesgo de Experimentar aumento de grado o readmisión, por Sexo", error=T,fig.width=8}
...
res_cox2 %>% 
  dplyr::select(-std.error,-estimate) %>% 
      knitr::kable(format = "html", format.args = list(decimal.mark = ".", big.mark = ","), digits=2,
               caption = paste0("Tabla. Modelos alternativos (incluyen fecha de término proceso)"),
...
```{r 21c, echo=T, paged.print=TRUE,eval=F}
#biostat3::survRate(Surv((diff_bet_proc), event_n>0) ~ factor(sex), data=part_17)
