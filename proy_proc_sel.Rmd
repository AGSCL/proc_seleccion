...
```{r 0, echo=T, paged.print=TRUE,eval=T}
...
Generamos columnas con un código único para identificar cada proceso de selección llamado `id_proc_sel`. Adicionalmente, dejamos que toda la información concerniente a cada proceso esté agrupado en la columna `títulos`, y que la columna `cargo` se replique para cada campo de cada proceso. Por el momento, nuestra base de datos se encuentra en formato largo.
...
part_3%>%
  dplyr::slice(1:400) %>% 
  knitr::kable(format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
                caption = paste0("Primeros 400 casos"),
                align =c('l',rep('c', 101)))%>%
     kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 12) %>%
     kableExtra::add_footnote(c("Nota. "),
                              notation = "none")%>%
...
part_7 <-
  ...
  dplyr::mutate(estado=dplyr::case_when(`proceso de seleccion`=="desierto"~"desierto",T~estado)) %>% 
  dplyr::mutate(`proceso de seleccion`=dplyr::case_when(`proceso de seleccion`=="desierto"~NA_character_,T~`proceso de seleccion`)) %>% 
  ...
  dplyr::mutate(`proceso de seleccion`=dplyr::case_when(`proceso de seleccion`=="desierto" & id_proc=="[a723 - a724 - a725 - a726] bibliotecario(a)"~"Interno",T~`proceso de seleccion`))%>% 
...
fuzzyjoin::stringdist_join(data.frame(name=part_12$sel1), data.frame(name=part_12$sel1), 
  ...
  slice(1:20) %>% 
    knitr::kable(format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
               caption = paste0("Comparación de nombres"),
               align =c('l',rep('c', 101)))%>%
    kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 12) %>%
  kableExtra::add_footnote(c("Nota. "),
                            notation = "none")%>%
...
De igual manera, se anonimizan los datos de personas seleccionadas, se ordena la base de datos por id_seleccionado y fecha de termino del proceso de selección. De igual manera, se eliminaron los espacios en blanco en el el estamento.
...
part_16 %>% group_by(id_seleccionado) %>% 
  ...
  dplyr::mutate(label=dplyr::case_when(`n()`==1~paste0("Seleccionados en un\nproceso\n",percent,"(n=",n,")"),
                                       `n()`==2~paste0("Seleccionados en 2\nprocesos\n",percent,"(n=",n,")"),
                                       `n()`==3~paste0("Seleccionados en 3\nprocesos\n",percent,"(n=",n,")")
                                       )) %>% 
...
table1_all <- suppressWarnings(compareGroups(sex ~ est+ grado_n+ fech_ter_num+ n_proc+ vesp, method= c(
                                            grado_n=2,
                                            est=3,
                                            fech_ter_num=2,
                                            n_proc=3,
...
La razón de tasas de densidad de incidencia de aumentar grado o profesionalizarse en mujeres es `r round(as.numeric(sex_irr$estimate[1]),2)` veces mayor que hombres (95% IC `r round(sex_irr$conf.int[1],2)`-`r round(sex_irr$conf.int[2],2)`; p`r ifelse(sprintf("%1.3f",sex_irr$p.value)=="0.000","<0.001",paste0("= ", sprintf("%1.3f",sex_irr$p.value)))`), aunque no es significativo.

<br>

```{r 20-survplot-sex-rec, dpi = 96, warning=F,message=F,fig.align='center', fig.cap="Riesgo Acumulado de Experimentar aumento de grado o readmisión, por Sexo", error=T,fig.width=8}
...
Atendiendo a que los riesgos entre hombres y mujeres se distribuyen de manera proporcional (Residuos Schoefeld: $X^2$ (`r data.frame(prop_haz$table)[1,2]`)= `r round(data.frame(prop_haz$table)[1,1],2)`  , p=`r round(data.frame(prop_haz$table)[1,3],4)`), se decidió hacer un análisis de supervivencia de cox.
...
De los resultados del análisis de supervivencia de cox se desprende que las mujeres tendrían `r round(res_cox$estimate,2)` mayores riesgos de experimentar un aumento de grado o profesionalización que hombres (95% IC `r round(res_cox$conf.low,2)`, `r round(res_cox$conf.high,2)`), aunque no es significativo.
...
En este caso, pasamos de `r nrow(part_17)` a `r nrow(dplyr::group_by(part_17,id_seleccionado)%>% dplyr::mutate(rn=row_number())%>% dplyr::filter(rn<2))`. Por tanto, aquí no consideramos los riesgos de un 3er o posterior proceso de selección. Lo anterior, para chequear si la exposición previa podría afectar los resultados de la siguiente.  
...
```{r 20-survplot-sex-rec-b, dpi = 96, warning=F,message=F,fig.align='center', fig.cap="Riesgo Acumulado de Experimentar aumento de grado o readmisión, por Sexo", error=T,fig.width=8}
...
Atendiendo a que los riesgos entre hombres y mujeres se distribuyen de manera proporcional (Residuos Schoefeld: $X^2$ (`r data.frame(prop_haz2$table)[1,2]`)= `r round(data.frame(prop_haz2$table)[1,1],2)`  , p=`r round(data.frame(prop_haz2$table)[1,3],4)`), se decidió hacer un análisis de supervivencia de cox.
...
De los resultados del análisis de supervivencia de cox se desprende que las mujeres tendrían `r round(res_coxb$estimate,2)` mayores riesgos de experimentar un aumento de grado o profesionalización que hombres (95% IC `r round(res_coxb$conf.low,2)`, `r round(res_coxb$conf.high,2)`).
