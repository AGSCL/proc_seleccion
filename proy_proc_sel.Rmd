...
part_7 <-
  part_6 %>% 
  dplyr::mutate(proc_sel=gsub("interno\\,", "interno", proc_sel)) %>%
...
data_list <- list(
  t = part_17_first_ev_naomit$t,
  right_cens_time = part_17_first_ev_naomit$right_cens_time,
  is_censored = part_17_first_ev_naomit$is_censored,
...
model {
  for (i in 1:N) {
    # Proper censoring mechanism
    is_censored[i] ~ dinterval(t[i], right_cens_time[i])
    
    # Weibull survival likelihood
    t[i] ~ dweib(r, mu[i])
    
    # PH parameterization: mu = exp(-(Xβ)/r)
    log(mu[i]) <- -(beta0 + beta1*sex[i] + beta2*fech_ter_num[i])/r
    
    # Hazard function
    hazard[i] <- r * pow(t[i], r-1) * exp(beta0 + beta1*sex[i] + beta2*fech_ter_num[i])
  }
  ...
  # Gamma prior for shape parameter (centered around 1)
  r ~ dgamma(1, 0.001)
}
"
...
coda_samples <- coda.samples(jags_model, 
  variable.names = c("beta0", "beta1", "beta2", "r", "hazard"), 
...
```{r 33-output2, echo=T, paged.print=TRUE,eval=T}
data_list2 <- list(
...
# Definir el modelo AFT en JAGS
model_string2 <- "
model {
   for (i in 1:N) {
     cens[i] ~ dinterval(logt[i], log(right[i])) # Información de censura
     logt[i] ~ dnorm(mu[i], tau)
     mu[i] <- beta0 + beta1 * sex[i] + beta2 * fech_ter_num[i]
   }
...
# Ajustar el modelo en JAGS
n_chains <- 9
jags_model2 <- jags.model(textConnection(model_string2), 
                         data = data_list2, 
                         n.chains = n_chains, 
                         n.adapt = 1000)
...
# Ejecutar el modelo
coda_samples2 <- coda.samples(jags_model2, 
                             variable.names = c("beta0", "beta1", "beta2", "sigma"), 
                             n.iter = 50000, thin = 10)

# Resumen de los resultados
print(summary(coda_samples2))

samples_df2 <- do.call(rbind, lapply(1:length(coda_samples2), function(i) {
  as.data.frame(as.mcmc(coda_samples2[[i]]))
}))

samples_df22 <- samples_df2 %>%
  mutate(
    hr_beta0 = exp(beta0),
    hr_beta1 = exp(beta1),
    hr_beta2 = exp(beta2)
...
# Resumir los coeficientes para obtener medias, desviaciones estándar y cuantiles
hr_summary2 <- samples_df22 %>%
  summarise(
    mean_beta0 = mean(hr_beta0),
    lower_beta0 = quantile(hr_beta0, 0.025),
    upper_beta0 = quantile(hr_beta0, 0.975),
...
# Crear un dataframe ordenado para facilitar la lectura e interpretación
tidy_hr2 <- tibble(
  term = c("Intercept", "Sexo(Mujer)", "Fecha de término"),
  estimate = c(hr_summary2$mean_beta0, hr_summary2$mean_beta1, hr_summary2$mean_beta2),
  conf.low = c(hr_summary2$lower_beta0, hr_summary2$lower_beta1, hr_summary2$lower_beta2),
  conf.high = c(hr_summary2$upper_beta0, hr_summary2$upper_beta1, hr_summary2$upper_beta2)
)
...
```{r exp, echo=T, paged.print=TRUE,eval=T}
# remover .RData
unlink("*.RData")
unlink("*.ini")
unlink(list.files(pattern = "\\.ini$", recursive = TRUE))

# guardar los datos
date_format <- format(Sys.Date(), "%Y_%m_%d")
save.image(paste0(date_format, ".RData"))
