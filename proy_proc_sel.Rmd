...
surv_data <- broom::tidy(fit_sex_rec)
fortify(surv_data, fun = "cumhaz") %>% 
ggplot(aes(x = time, y = cumhaz, color = strata, fill = strata, group=strata)) +
  geom_step() +
  ...
  theme_classic() +
  labs(x = "Tiempo (días)", y = "Riesgo acumulado", color = "Profesionalización/\nAumentar grado") 
...
surv_data2 <- broom::tidy(fit_sex_rec2)	
fortify(surv_data2, fun = "cumhaz") %>% 
ggplot(aes(x = time, y = cumhaz, color = strata, fill = strata, group=strata)) +
  geom_step() +
  scale_color_manual(values = c("skyblue4","gray50"), labels = c("Hombre", "Mujer")) +
  scale_fill_manual(values = c("skyblue4","gray50"), labels = c("Hombre", "Mujer")) +	
